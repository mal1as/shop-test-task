...
gradle bootRun
```
3. Swagger API - http://localhost:8337/swagger-ui/index.html#/
...
Паттерн для налогового номера реализован через Regex и сохранения паттерна в базу, а потом проверке через similar to.

### Платежные процессоры

Платежные процессоры унифицированы через интерфейс с одним методом для проведения оплаты.
Этот главный метод вызывает методы описанные в задании методы и по сути является удобной оберткой.
